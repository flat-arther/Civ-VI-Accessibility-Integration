...
- 2026-05-19: Added `BuildMovementResultSpeech(unit, targetX, targetY, turnsToArrival)` for future movement-result events. It returns localized `Moved, n moves left`, `Stopped short, n turns till arrival`, or `Stopped short`; Civ VI uses `GetMovementMovesRemaining()` directly for display movement, so no Civ V `MOVE_DENOMINATOR` equivalent is used.
- 2026-05-19: Added `EventSubs_CAI.lua` and included it from `WorldInput_CAI.lua`. It subscribes to `Events.UnitMoveComplete` for local-player units, reads `UnitManager.GetQueuedDestination(unit)`, re-runs `UnitManager.GetMoveToPathEx(...)` for remaining queued destinations, and speaks `BuildMovementResultSpeech(...)`.
