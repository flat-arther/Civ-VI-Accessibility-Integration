...
- ProductionPanel per-tab body refactor implemented in `src/UI/inGame/ProductionPanel_CAI.lua`: Production, Purchase Gold, and Purchase Faith now each own a dedicated CAI treeview, while Queue keeps its dedicated list. The old shared-body / last-built-tab orchestration was removed; tab switches now show the already-built per-tab body and rebuild only that tab's rows from live data. Category headers still resolve through `categoryListsByMode[listMode][categoryKey]`, category nodes now reinitialize from the matching vanilla expanded/collapsed state whenever a tree tab is refreshed, and current production is now shown only on the Production tree and Queue list to match vanilla. Pending in-game verification.
- Shared widget focus fallback bug traced in `src/UI/uiManager/widgetTemplateHelpers.lua` and `src/UI/uiManager/UIScreenManager.lua`: containers could keep a stale `FocusedChild` reference after that child became hidden, because default-child lookup and manager focus walking reused `FocusedChild` without validating visibility first. Patched by clearing hidden stale focused children before descending, both in shared container default-child helpers and in the manager's focused-widget / focus-path traversal. Verified in-game.
- ProductionPanel category-header follow-up fixed in `src/UI/inGame/ProductionPanel_CAI.lua`: the CAI category nodes were still reading old single captured refs like `m_vanilla.districtList` and `m_vanilla.unitList` even though vanilla repopulates several list modes in one `View(...)` pass. Category header captures, label reads, unit-group toggles, and CAI-to-vanilla expand/collapse now resolve through `categoryListsByMode[listMode][categoryKey]` so each active CAI tab talks to the correct live vanilla header/list instance. Pending in-game verification.
- ProductionPanel category-state follow-up simplified in `src/UI/inGame/ProductionPanel_CAI.lua`: CAI category widgets now read vanilla expanded/collapsed state only once when the category nodes are first built, then keep normal CAI tree state afterward. The only remaining bridge is the category `OnToggleExpanded` callback, which drives vanilla `OnExpand(...)` / `OnCollapse(...)` when the CAI user toggles a category; earlier refresh-time mirroring experiments and vanilla header wrappers were removed. Pending in-game verification.
- ProductionPanel CAI received the same structural cleanup pass as GovernmentScreen in `src/UI/inGame/ProductionPanel_CAI.lua`. Runtime/UI/vanilla-capture state is now grouped into `m_state`, `m_ui`, and `m_vanilla`; dead category-builder and last-built scaffolding were removed; category labels now come only from live captured vanilla headers; ordinary tab switching still syncs through `LuaEvents.ProductionPanel_ListModeChanged(...)`; delayed CAI opening now uses `ProductionPanel_Open` only as an open-pending flag and waits for the first real `ProductionPanel_ListModeChanged(...)` before syncing/pushing the CAI panel; and the speculative `CityWorkersChanged` refresh hook was dropped in favor of `CityProductionChanged`, `CityProductionUpdated`, and `CityProductionQueueChanged`. Verified in-game.
...
- ProductionPanel city-open timing was rechecked against base Lua. The confirmed order is `Open()` -> `LuaEvents.ProductionPanel_Open()` -> return from `Open()` -> vanilla `m_tabs.SelectTab(...)` -> vanilla `ProductionPanel_ListModeChanged(...)`. CAI therefore uses `ProductionPanel_Open` only as an open-pending flag and waits for the first `ProductionPanel_ListModeChanged(...)` to sync and push the accessibility panel on the real final tab. Pending in-game verification.
...
- ProductionPanel CAI refactor cleanup: wrapper state was reorganized into `m_state` / `m_ui` / `m_vanilla`, delayed opening now uses `ProductionPanel_Open` as an open-pending signal and `ProductionPanel_ListModeChanged` as the real open handshake, and the refresh hook set was tightened to production-changed / production-updated / production-queue-changed only. Pending in-game verification.
...
- Verify Production tab category rows inherit the correct initial expanded/collapsed state from vanilla when the CAI category nodes are first built, and afterward CAI expand/collapse stays under user control without snapping back on refresh while still expanding/collapsing the matching vanilla header when toggled from CAI.
...
- Verify only Production and Queue expose the current-production widget; Purchase Gold and Purchase Faith must not show it.
- Verify each CAI tree tab keeps its own body widget and no longer reuses a shared tree between Production / Purchase Gold / Purchase Faith.
...
- Verified: when a container remembers a focused child and that child becomes hidden, reopening or restoring focus now lands on the next visible/default child instead of the hidden stale child.
