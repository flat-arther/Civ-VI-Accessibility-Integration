...
- `navCursor.lua` now keeps cursor event payloads scalar-only: `LuaEvents.CAICursorMoved` passes `plotId` instead of live plot objects, `CAICursorSnapToPlot` uses plot ids, and selection/scanner jumps use a new `LuaEvents.CAICursorJump(plotId)` path that speaks the jump direction before moving.
...
- 2026-05-18: Investigated post-combat summary options for `UnitPanel`. `Events.Combat` fires after combat resolves with the same `CombatResultParameters` table shape that preview uses. Post-combat speech should rebuild attacker / defender / interceptor / anti-air names from nested `ID` payloads plus `LOCATION`, and derive concise outcomes from `DAMAGE_TO`, `FINAL_DAMAGE_TO`, `DEFENSE_DAMAGE_TO`, `FINAL_DEFENSE_DAMAGE_TO`, `DEFENDER_CAPTURED`, `ATTACKER_ADVANCES`, and `LOCATION_PILLAGED` rather than from live preview controls.
- 2026-05-18: `UnitPanel_CAI.lua` now owns post-combat result speech through `Events.Combat`. It speaks only visible/local non-WMD combats, skips nuke / WMD results by checking `WMD_TYPE` / `WMD_STATUS`, mirrors vanilla plot-target naming instead of a CAI plot-coordinate fallback, adds support-damage wording for interceptor / anti-air, speaks mixed defender-plus-wall damage, and distinguishes successful vs failed pillage plus wall-destruction outcomes.
- 2026-05-18: Investigated a new combat-preview regression after the compact string reformat. In `UnitPanel_CAI.lua`, `GetTargetPreviewDamageText()` currently returns wall damage early for visible walled-city targets, so preview speech drops simultaneous city HP damage instead of speaking both values in the target-damage clause.
- 2026-05-18: Combat preview damage speech now uses unsigned dealt-damage values. Walled-city target damage stays in the existing target clause; ordinary wall hits append wall damage after city damage, while a wall-breaking hit says `destroys walls`.
- 2026-05-18: Investigated vanilla `WorldInput.lua` movement preview from the ground up. `RealizeMovementPath(showQueuedPath)` is a visual renderer, not a text-summary API: it chooses between `MovementGood`, `MovementQueue`, `MovementBad`, and `MovementFOW`, uses numbered turn-break markers plus pip/minus variants, switches to attack-range focus for implicit ranged attacks, supports swap previews and tunnel segment splits, and restores queued-path rendering on cancel/selection when applicable. Durable notes were added to `docs/game-api.md` for the CAI movement-info redesign.
- 2026-05-18: Rebuilt CAI movement interface info around Civ VI `GetMoveToPathEx(...)`. Arrival turn now comes from the final `pathInfo.turns` entry instead of path-node count, path directions use `CAIHexCoordUtils.stepListFromPath(...)`, movement strings are localized in `cai_text_ui.xml`, visible-prefix fog speech says `Then unexplored`, tunnel/obstacle/swap/queued states are surfaced, and active move-to previews that would immediately start combat request `LuaEvents.CAISpeakCombatPreview()` instead of speaking movement. Needs in-game verification for ordinary one-turn, multi-turn, fog, tunnel, blocked, queued, and immediate-combat move previews.
- 2026-05-18: Expanded movement interface info toward the Civ V announcement style using only confirmed Civ VI data. MP spent / remaining is intentionally omitted because dumped `GetMoveToPathEx(...)` keys expose no per-node MP. Added localized hints for ZOC entry, embark/disembark, will-declare-war, and delayed attack-after-move paths; ordinary successful moves now lead with arrival timing instead of a generic `Valid move` phrase.
- 2026-05-19: Fixed movement fog wording. `IsVisible(...)` is still used to match vanilla's FOW movement lens, but spoken path geometry now follows revealed terrain via `IsRevealed(...)`; revealed-but-not-visible paths say `Path enters fog`, while genuinely unrevealed paths say `Path enters unexplored territory` and append `Then unexplored`.
- 2026-05-19: Simplified delayed-combat movement speech. Paths that will encounter combat after movement now say only `Encounters combat in n turns` and suppress the route breakdown; ordinary movement directions no longer include a `Path:` prefix.
- 2026-05-19: Added movement-result speech for future movement events. It returns localized `Moved, n moves left`, `Stopped short, n turns till arrival`, or `Stopped short`; Civ VI uses `GetMovementMovesRemaining()` directly for display movement, so no Civ V `MOVE_DENOMINATOR` equivalent is used.
- 2026-05-19: Added `EventSubs_CAI.lua` and included it from `WorldInput_CAI.lua` for tutorial movement restriction subscriptions.
- 2026-05-19: Investigated how sighted queued movement is owned in vanilla world input. `WorldInput.lua` and `Civ6Common.lua` show queued destinations and re-render them through `UnitManager.GetQueuedDestination(unit)` plus `RealizeMovementPath(true)`, but move confirmation still submits the same plain `UnitManager.RequestOperation(unit, UnitOperationTypes.MOVE_TO, tParameters)` path as ordinary movement, with no exposed Lua-side `Shift` or queue modifier branch. Current conclusion: queue creation itself is engine-owned, while Lua only previews, restores, and inspects queued state after the fact.
- 2026-05-19: Added `UnitWaypoints_CAI.lua` as a WorldInput-owned singleton cache for selected-unit queued-path waypoints. It tracks selection and move completion, derives waypoint plot ids from `GetMoveToPathEx(...)` turn breaks, exposes `GetUnitWaypoints()` / `IsWaypointPlot(plotId)` through `ExposedMembers.CAIInfo`, adds `Waypoint` plot-tooltip speech in `PlotToolTip_CAI.lua`, and adds a dynamic `Waypoints` world scanner category through `WorldScannerCategory_waypoints.lua`.
- 2026-05-19: Extended waypoint speech for units. `UnitWaypoints_CAI.lua` now exposes `GetNextUnitWaypoint()`, and `UnitPanel_CAI.lua` speaks `Next waypoint: <direction>` from the unit's plot using `hexCoordUtils_CAI.lua` direction math. Summary speech places it right after activity, and queued-path reads now speak next waypoint before the full queued path.
- 2026-05-19: Fixed waypoint caching for exhausted queued units. `GetMoveToPathEx(...)` turn-break parsing could emit the unit's current plot as the first waypoint, which made unit summary and queued-path speech say `Next waypoint: Here`. `UnitWaypoints_CAI.lua` now skips the current unit plot when caching turn-break waypoint ids.
- 2026-05-19: Restored full queued-path direction speech in `UnitPanel_CAI.lua` reads. Shared `BuildMovementSpeech(...)` had been suppressing the `Path: ...` line whenever delayed-combat `attackAfterMoveText` existed, which unintentionally removed route breakdowns from explicit queued-path reads too. Queued paths now keep the full direction breakdown while ordinary move-preview delayed-combat speech stays compact.
- 2026-05-20: Added `MovementActions_CAI.lua` as a WorldInput-owned helper for adjacent quick-move hotkeys plus shared `MOVE_TO` primary-action confirmation. `Alt+Q/E/A/D/Z/C` and Alt+numpad mirrors now analyze adjacent targets from the selected unit tile, reuse `BuildMovementPathInfo(...)` / `BuildMovementSpeech(...)`, arm immediate melee combat on first press, and commit through one silent cursor-sync plus the same vanilla `OnMouseMoveToEnd()` path used by mouse move confirmation on second press or on ordinary valid moves.
- 2026-05-21: Movement-result ownership moved into `MovementActions_CAI.lua`. It now mirrors the older Civ V CAI single-flight pattern more closely: one active pending move snapshot is overwritten on each new commit, `Events.UnitMoveComplete` remains the fast path for actual step completion, and `WorldInput_CAI.lua` now polls `MovementActions_CAI:UpdatePendingMovementResult()` from `OnUpdate()` as a reveal-announcement-style watcher. Pending movement resolves only when live game state positively proves either arrival at the committed target or a matching queued destination, and pending is explicitly cleared on local-turn begin and shutdown so stale state cannot leak into later turns.
- 2026-05-20: Fixed a movement-failure regression in `interfaceInfoHelpers_CAI.lua`. Bad-path diagnostics had started using `pathInfo.plots[1]` as the origin even when no path existed, which replaced the unit's real start plot with the target plot and made coastal embark failures fall through to generic `Cannot move there`. CAI now always diagnoses failures from `unit:GetPlotId()` and again speaks the expected embark tech for coast targets; the cross-area land diagnostic was also restored so `Cartography` can still be named when appropriate.
- 2026-05-20: Refined movement ZOC speech in `interfaceInfoHelpers_CAI.lua`. Non-queued movement preview still uses `UnitManager.GetReachableZonesOfControl(unit, true)`, and path analysis now distinguishes `Intersects zone of control` for any earlier path node from `Ends at zone of control` when the destination plot itself is in that reachable ZOC set. Spoken output is mutually exclusive: destination ZOC takes priority over intersection.
- 2026-05-20: Rebalanced movement preview verbosity between cursor motion and explicit interface info. In move mode, ordinary cursor reads now keep only the compact summary from `BuildMovementSpeech(...)`, while explicit `Space` reads add the step-by-step path breakdown plus fog / unexplored path lines. Failure, turn, obstacle, tunnel, ZOC, war/combat, and blocked reporting remain in ordinary cursor movement speech.
- 2026-05-20: Detailed path-step speech now honors turn breaks in both places that expose the full path. Explicit move preview splits visible steps into per-turn segments using `pathInfo.turns`, while queued-path speech splits cached visible steps at queued waypoint markers; both join segments with localized `then`.
- 2026-05-19: Reworked queued-path caching around one shared source of truth. `UnitWaypoints_CAI.lua` now caches the full queued path as `{ PlotId, IsWaypoint }` entries plus cached arrival turn, exposes `GetQueuedPath()`, `GetQueuedPathArrivalTurn()`, `IsQueuedPathPlot(plotId)`, and derives waypoint queries from those flags. `UnitPanel_CAI.lua` queued-path reads now rebuild speech from the cached path instead of re-running `GetMoveToPathEx(...)`, and the scanner category was renamed to `Queued path` with `Full path` and `Waypoints` subcategories that label items through `RequestPlotInfo(...)`.
- 2026-05-19: Simplified queued-path cache ownership. `UnitWaypoints_CAI.lua` no longer tracks player ids, unit ids, or init state. It now stores only one cached queued path for the currently selected unit, clears/repopulates on `UnitSelectionChanged`, refreshes on `UnitMoveComplete` only when that moved unit is still selected, and public getters self-clear if the selected unit no longer has a queued destination.
- 2026-05-21: Refined queued waypoint semantics. `UnitWaypoints_CAI.lua` still reserves cached `IsWaypoint` markers for real turn breaks only, but semantic waypoint queries now treat the final queued destination as a waypoint too. `GetNext()`, `GetUnitWaypoints()`, `IsWaypointPlot()`, and the queued-path waypoint scanner all include the final destination without marking it as a cached turn-break waypoint.
- 2026-05-19: `UnitFlagManager_CAI.lua` now adds owned-unit queued movement to its default spoken bucket. It uses `UnitManager.GetQueuedDestination(unit)` plus shared hex-direction math to say `Queued movement: <direction>` for local-player units, and that same text now participates in same-tile exact-match grouping so stacked units with different queues are not merged together in plot tooltip speech.
- 2026-05-19: `PlotToolTip_CAI.lua` cursor-move speech now drops the automatic relative-coordinate prefix. It also adds an `isFog` plot-info helper at the start of the cursor-move bucket: plots that are revealed but not currently visible speak `Fog`, while unrevealed plots still use the existing terrain-name fog-of-war fallback.
- 2026-05-19: `interfaceInfoHelpers_CAI.lua` now uses a CAI-owned movement failure diagnostic instead of the temporary `CanStartOperation(... MOVE_TO ...)` probe. Failed move previews diagnose tutorial restriction, visible blockers, land/water attack mismatches, sea-to-land travel, impassable terrain, foreign territory, unit-specific embark tech, and `Cartography`-gated ocean travel from start/target plot state. Builders now report `Sailing`, traders report `Celestial Navigation`, and other land units report `Shipbuilding` before CAI escalates ocean or cross-area failures to `Cartography`.
- 2026-05-19: Cleaned up stale movement embark diagnostics in `interfaceInfoHelpers_CAI.lua`. Embark/disembark preview speech is now based only on start/end plot water state, and failed movement no longer infers embark or `Cartography` from land-to-land area changes or intermediate path tiles.
- 2026-05-19: Cleaned up stale delayed-combat movement speech in `interfaceInfoHelpers_CAI.lua`. Future hostile contact now speaks only `Encounters combat this turn` or `Encounters combat in n turns` from real hostile destination state plus arrival turn, and the old `Enemy at destination` fallback was removed from movement previews.
- 2026-05-19: Cleaned up the remaining movement-failure fallback overlap in `interfaceInfoHelpers_CAI.lua`. Failed move previews now use only the resolved failure diagnostic, so water-tech failures no longer stack `Requires embark` on top of `Requires: <tech> technology`.
- 2026-05-19: Removed generic embark/disembark movement speech from `interfaceInfoHelpers_CAI.lua` entirely. Requirements are now spoken only through the failure diagnostic path, not through ordinary movement or fog-preview summaries.
- 2026-05-19: Fixed implicit ranged-attack classification in `interfaceInfoHelpers_CAI.lua`. `MOVE_TO` preview now checks ranged-attack target eligibility before path-failure handling, so hostile tiles for ranged units speak `Ranged attack` on cursor reads instead of incorrect movement-failure lines like `Blocked by unit`, while explicit `Space` preview still delegates to combat preview speech.
- 2026-05-19: Realigned `BuildMovementPathInfo(...)` to vanilla `RealizeMovementPath(...)` branch order: queued destination, invalid plot, swap, `GetMoveToPathEx(...)`, implicit ranged-attack on real multi-plot paths, then queue / restricted / FOW / good classification. CAI now also distinguishes vanilla's two `MovementBad` shapes: restricted paths still keep path details, while true no-path targets return only failure speech and no fake enemy/path follow-up.
...
- Verify grouped same-tile unit speech merges only exact spoken matches by owner, name, queued movement, formation suffix, rounded health percent, and visible status.
- 2026-05-20: `UnitFlagManager_CAI.lua` now reuses `RequestUnitInfo(unitID, { "NextWaypoint" }, ownerID)` for local-player queued movement, so ambient unit-flag and plot `units` speech should say `Next waypoint: <direction>` instead of only the final queued destination. The older queued-destination direction helper is no longer used there.
- 2026-05-21: Audited vanilla `UnitFlagManager` coverage across base, `BarbarianClansMode`, `PiratesScenario`, `CivRoyaleScenario`, and `PolandScenario`. Current CAI unit-flag speech still omits vanilla flag/tooltip data for promotion-or-levy badge state, religion badge/name, archaeology tooltip lines, levy-owner turns text, air-capacity popup info, hero/special marker state, and the DLC-specific barbarian-clan, pirates, and Civ Royale additions.
- 2026-05-21: `UnitFlagManager_CAI.lua` now selects the matching vanilla include by active ruleset / mode (`CivRoyale`, `Pirates`, `Barbarian Clans`, else default) and adds short spoken unit-flag helpers for promotion-or-levy, religion, archaeology, aircraft capacity, hero / threat / flagship markers, Barbarian Clans clan/status info, and the Pirates Buccaneer label.
...
- Verify the new movement-failure diagnostic:
- failed targets should prefer specific reasons in this order: tutorial, visible blocker/domain mismatch, sea-to-land mismatch, impassable terrain, foreign territory, requires embark tech for that unit, requires `Cartography`, generic fallback.
- targeting water from land with no valid path should speak the correct embark unlock for that unit: `Sailing` for builders, `Celestial Navigation` for traders, `Shipbuilding` for other land units.
- targeting land across a different land area with no valid path should speak `Requires: Cartography technology`.
- Verify movement ZOC speech:
- non-queued movement preview should say `Ends at zone of control` when the destination matches `GetReachableZonesOfControl(unit, true)`; otherwise, if any earlier path plot matches, it should say `Intersects zone of control`. Never speak both lines for one preview.
- Verify movement preview verbosity split:
- ordinary move-mode cursor speech should omit path-step and fog / unexplored path-detail lines while still speaking failure, turn, obstacle, tunnel, ZOC, war/combat, and blocked-state info.
- explicit `Space` interface info in move mode should add the full path-step breakdown plus `Path enters fog` / `Path enters unexplored territory` and trailing `Then unexplored` when applicable.
- Verify turn-break path wording:
- explicit move preview and queued-path reads should split the visible path at each end-of-turn break and join segments as `..., then, ...`, for example `2 E, then, 1 NW, 1 E`.
- Verify quick move and shared move confirmation:
- `Alt+Q/E/A/D/Z/C` and Alt+numpad mirrors should move the selected unit to the adjacent tile from the unit's own plot, not from the CAI cursor.
- valid non-combat targets should commit immediately through the same vanilla move path as move mode.
- immediate melee combat targets should speak combat preview plus `Press again to confirm combat` on first press, then commit only when the same unit and same target are pressed again.
- `MOVE_TO` Return should use the same armed-confirmation flow as quick move, and only reach `OnMouseMoveToEnd()` at the final commit step.
- selection changes, leaving move mode, Escape cancel, turn change, failed activation, successful commit, and different-target attempts should all clear the pending combat confirmation.
