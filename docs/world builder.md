...
2. **Per-player state never created.** Cursor, message buffer, scanner, and reveal all build state through the shared `PlayerStateManager` (`PlayerStateManager_CAI.lua`), keyed on `GetActivePlayerID()` → `Game.GetLocalPlayer()` → nil in WB → `Get()` rejects it (`< 0`), so nothing initialized. Fixed with an **observer fallback**: new `GetViewingPlayerID()` / `IsObserverView()` in `caiUtils.lua`. Civ VI may expose `PlayerTypes.OBSERVER`, the positive sentinel `1000`, through either `Game.GetLocalPlayer()` or `Game.GetLocalObserver()` depending on context. The helpers recognize both forms and use the sentinel as a stable state key (`Players[1000]` is nil). `PlayerStateManager`, the scanner context, and reveal key on this, so all four systems initialize in WB/observer.

**Ownership under an observer collapses completely to neutral** (observers own nothing and see everything). `IsObserverView()` gates the classifiers: Surveyor `IsOwnOrTeamUnit` → false, `IsEnemyUnit` → false, `IsNeutralUnit` → true, `IsKnownPlayer` → true, resources → see-all; the World Scanner's `Utils.CanKnowPlayer` → true and `Utils.GetTeamStance` → neutral. This includes barbarian units and city-states: their factual owner/type can still be spoken, but observer-relative grouping is always neutral. World Builder unit enumeration scans every player slot and does not require `Player:IsAlive()`, because initialized map-editor players are not necessarily alive in the gameplay sense. Unit visibility is also see-all in observer mode. Reveal keys on the observer but has nothing to announce in a fully-revealed map. Cursor audio (`cursorAudio_CAI.lua`) also gates its terrain/fog sounds on `IsPlotRevealed`/`IsPlotVisible`; both return see-all when `IsObserverView()`.

Cursor zone updates follow the same visibility order. While Set Visibility is armed, the selected player's reveal model controls whether continent, owner, territory, and city zones can update. With every other World Builder tool, the observer is see-all. Owner-zone identity resolves directly from the configured plot owner because observer `1000` has no player/diplomacy object.
