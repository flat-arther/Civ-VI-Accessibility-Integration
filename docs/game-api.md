...
- `Assets/UI/PartialScreens/CityStates.lua` keeps the send-envoys list and confirm drawer alive in one screen rather than opening a second modal. `OnConfirmPlacement()` submits the pending envoy operations, clears the vanilla pending-change table, and either closes the screen when no envoys remain or leaves the list open for more assignments. For CAI focus handling this means a passive tree rebuild after confirm must preserve the currently selected city-state even when focus is sitting on the separate confirm button; otherwise the slider becomes hidden for one tab step because its visibility is keyed off the selected city-state, not off the last tree row focus.
...
# CAI positional raw audio

- `CAI_AudioDefinitions.IsPositional` marks raw sounds that must be played through `CAIAudioManager:PlayAtPlot(...)` / `QueueSoundAtPlot(...)`; ordinary `Play(...)` / `QueueSound(...)` reject them so a positional handle cannot reuse stale plot parameters. `SCANNER_BEACON` resolves `sounds/UI/scanner_beacon.wav` under the `BEACONS` tag and is the first positional definition.
- Spatial audio is an in-game-only manager capability. The shared audio manager loads raw sound handles normally but does not initialize spatial state during its general initialization. `WorldInput_CAI.lua` explicitly calls `InitializeSpatialAudio()` from `RegisterCAIEvents()`, which runs only through the wrapped `Events.LoadScreenClose` game-view path. Runtime logging confirms `UI.IsInGame()` can still return false at that valid callback, so the spatial initializer must not reject the call using that query; the WorldInput lifecycle boundary itself is authoritative. The initializer includes `hexCoordUtils_CAI` if needed, initializes the listener, and initializes every already-loaded positional handle. `PlayAtPlot(...)` and `QueueSoundAtPlot(...)` only verify that this lifecycle initialization completed; they do not initialize spatial audio lazily. Front-end playback continues to use ordinary raw audio without invoking spatial DLL methods.
- Each positional handle enables DLL spatialization and initializes source position `(0, 0, 0)`, normalized direction `(0, 0, -1)`, zero velocity, no native attenuation, centered pan, and normal pitch before playback. The listener initializes at the same origin with direction `(0, 0, -1)`, up `(0, 1, 0)`, and zero velocity. Source/listener positions remain co-located because CAI deliberately supplies the Civ V hex encoding rather than combining it with native bearing or attenuation math.
- Runtime initialization confirms the DLL spatial functions are callable at the WorldInput load-screen-close boundary, but `CAI.AttenuationModel` can still be nil there even though the IDE helper documents the enum table. Pass the documented numeric value `0` to `CAI.SetSoundAttenuationModel(...)` for no attenuation instead of indexing `CAI.AttenuationModel.None`; indexing the missing table aborts initialization after the source direction and velocity calls.
- `CAIHexCoordUtils.displacement(...)` applies nearest-map-wrap folding and odd-row half-column correction. `spatialAudioParameters(...)` maps that displacement to the Civ V beacon model: pan is `clamp(dcol / 12, -1, 1)`; pitch is one semitone per row capped at one octave through `2 ^ (clamp(drow, -12, 12) / 12)`; volume fades linearly from 1 to 0 over the requested maximum `Map.GetPlotDistance(...)`, defaulting to 30 hexes.
- `PlayAtPlot(soundId, sourcePlotOrId, options)` accepts a Plot or plot id. `options.ListenerPlot` may supply a Plot/id scan origin; otherwise the manager resolves the current CAI cursor live. `options.MaxDistance` overrides the 30-hex default. Active positional sounds recompute from live plot ids during the manager update, so long or looping sounds follow cursor movement. Queued positional sounds cache ids, not Plot objects, and resolve the default cursor listener at dispatch time.
- Unless `SkipIfPlaying` is explicitly requested, `PlayAtPlot(...)` stops an already-playing positional handle, applies the new plot's pan/pitch/volume after that stop, and then calls `PlaySound(...)`. The order matters because stopping may reset per-handle parameters; applying them before the stop can make rapid scanner focus changes replay at default pitch instead of the newly focused plot's pitch.
- Runtime traces at 49 hexes showed the manager calculating spatial gain `0` and `CAI.GetSoundVolume(...)` returning `0` both before and immediately after `CAI.PlaySound(...)`, while the spatial beacon remained audibly full-volume. Reapplying the same composed volume after starting the voice did not change the output. Lua-side distance calculation and stored handle volume are therefore correct; the remaining discrepancy requires investigation in the DLL spatial playback implementation. The post-play reapplication was removed because it had no effect.
- Effective positional volume is the sound's base gain multiplied by its live tag-setting volume and current spatial distance gain. `SetSoundVolume(...)` changes the base gain, so later tag or cursor updates preserve all three components.
...
- `ScannerBeaconEnabled` is a World scanner checkbox setting, enabled by default and sorted before the other scanner settings. The shared scanner beacon helper reads it live, so disabling it suppresses both item-focus beacons and End-key beacon replays without suppressing scanner speech.
