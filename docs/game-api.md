...
- `ProductionPanel.lua` tab-open sequencing matters:
  ...
  - `include("ProductionPanel")` runs `Initialize()` immediately. That means `LuaEvents.CityPanel_PurchaseGoldOpen.Add(OnCityPanelPurchaseGoldOpen)`, `LuaEvents.CityPanel_PurchaseFaithOpen.Add(...)`, and the `m_tabs.AddTab(..., OnTabChange...)` callback captures all happen before CAI wraps those globals.
  - Reassigning `OnCityPanelPurchaseGoldOpen`, `OnCityPanelPurchaseFaithOpen`, or `OnTabChangePurchase*` later does not replace the already-registered LuaEvent handlers or the callback functions already stored inside `m_tabs`.
  ...
  - CAI-side tab state therefore must survive close/open teardown long enough to mirror the requested tab after vanilla setup, and CAI should treat `ProductionPanel_ListModeChanged` as the source of truth for the currently active vanilla tab.
...
- Vanilla diplomacy flow:
  ...
  - CAI now replaces `LeaderView` with `UI/inGame/LeaderView_CAI.lua` through `ReplaceUIScript`.
  ...
  - The CAI dialog is rebuilt after `ShowFirstMeetingLeader(...)`, `ShowWarLeader(...)`, `ShowRefusePeaceLeader(...)`, and `OnTalkToLeader(...)`, so it reflects the latest vanilla popup text and visible buttons.
  ...
  - The Gossip tab shows visible gossip from the last 100 turns, split into "last ten turns" and older items, with a "new" indicator for this turn or last turn.
