...
UPDATE LocalizedText SET Text = '按 Shift 加 Tab 打开启动栏并选择科技树。也可以按 Control 加 Shift 加 R。' WHERE Tag = 'LOC_META_46a_BODY' AND Language = 'zh_Hans_CN';
...
UPDATE LocalizedText SET Text = '按 Shift 加 Tab 打开启动栏并选择宗教界面及其中的万神殿选项。也可以按 Control 加 L。' WHERE Tag = 'LOC_META_129b_BODY' AND Language = 'zh_Hans_CN';
UPDATE LocalizedText SET Text = '按 Shift 加 Tab 打开启动栏并选择 [ICON_GreatPerson] 伟人界面。也可以按 F3。在伟人树中，使用方向键，或输入伟人姓名的一部分，找到可以招募的候选人。按 Tab 直到焦点移到“招募”，然后按 Enter。' WHERE Tag = 'LOC_META_136b_BODY' AND Language = 'zh_Hans_CN';
UPDATE LocalizedText SET Text = '世界排名会追踪每种胜利条件的状态。' WHERE Tag = 'LOC_META_119_BODY' AND Language = 'zh_Hans_CN';
UPDATE LocalizedText SET Text = '按 Shift 加 Tab 打开启动栏并选择世界排名。也可以按 F8。' WHERE Tag = 'LOC_META_119a_BODY' AND Language = 'zh_Hans_CN';
...
UPDATE LocalizedText SET Text = '如果需要进一步了解教程中的内容，请按 Shift 加 Tab 打开启动栏并选择文明百科。也可以按 F1。“无障碍模组”章节还记录了 CAI 控件、游戏命令和界面导航方式。' WHERE Tag = 'LOC_META_123_BODY' AND Language = 'zh_Hans_CN';
...
UPDATE LocalizedText SET Text = '按 Shift 加 Tab 打开启动栏并选择政体界面。也可以按 Control 加 P。' WHERE Tag = 'LOC_META_93b_BODY' AND Language = 'zh_Hans_CN';
...
UPDATE LocalizedText SET Text = '按 Shift 加 Tab 打开启动栏并选择市政树。也可以按 Control 加 Shift 加 C。' WHERE Tag = 'LOC_META_99b_BODY' AND Language = 'zh_Hans_CN';
...
UPDATE LocalizedText SET Text = '本教程的其余部分将帮助你取得第一次统治胜利。[NEWLINE][NEWLINE]统治胜利的目标，是让你的文明成为最后一个仍占领己方 [ICON_Capital] 首都的文明。[NEWLINE][NEWLINE]在本教程中，你必须占领对手的 [ICON_Capital] 首都，同时阻止对方夺取你的首都。[NEWLINE][NEWLINE]三项可选目标会帮助你实现这一点。按 Shift 加 Tab 打开启动栏并选择教程目标列表。也可以按 Control 加 Shift 加 T。教程目标也会显示在通知面板中。' WHERE Tag = 'LOC_META_122_BODY' AND Language = 'zh_Hans_CN';
...
UPDATE LocalizedText SET Text = '游戏过程中，请考虑如何与邻国相处，并仔细观察他们以了解其策略。按 Shift 加 Tab 打开启动栏并选择外交丝带列表，或按 F4，打开已遇见领袖的列表，然后在一位领袖上按 Enter，打开与其相关的详细外交界面。' WHERE Tag = 'LOC_META_157_BODY' AND Language = 'zh_Hans_CN';
