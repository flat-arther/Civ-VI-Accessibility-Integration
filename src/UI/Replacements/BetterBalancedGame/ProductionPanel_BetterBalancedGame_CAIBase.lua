...
-- Verbatim copy of Better Balanced Game's ui/replacements/productionpanel_bbg.lua
-- (BBG workshop item 2312050357). BBG registers that file only in a losing
-- <ReplaceUIScript>, never in <ImportFiles>, so CAI cannot resolve it with a
-- runtime include(). We vendor it here, register it in CAI's <Files> +
-- <ImportFiles id="CAIInGame">, and include THIS file from ProductionPanel_CAI.lua
-- when BBG is active, so BBG's production-list changes (Warrior Monk removal,
-- Water Mill / Palgum dynamic prereq) are present for the CAI accessibility layer
-- to wrap. Re-copy from BBG when BBG updates this file.
-- ===========================================================================
