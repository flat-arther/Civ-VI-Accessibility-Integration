...
-- Verbatim copy of Better Balanced Game's ui/replacements/worldrankings_bbg.lua
-- (BBG workshop item 2312050357). BBG registers that file only in <Files> and a
-- losing <ReplaceUIScript>, never in <ImportFiles>, so CAI cannot resolve it with
-- a runtime include(). We vendor it here, register it in CAI's <Files> +
-- <ImportFiles id="CAIInGame">, and include THIS file from WorldRankings_CAI.lua
-- when BBG is active, so BBG's Traditional Domination tab is present for the CAI
