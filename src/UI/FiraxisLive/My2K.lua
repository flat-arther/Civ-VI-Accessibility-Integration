...
local H = mgr and mgr.WidgetTemplateHelpers or WidgetTemplateHelpers
local CAI_My2KDialog = nil

local function CAI_GetText(control, fallback)
...
local function CAI_RemoveCurrentDialog(suppressRefocus)
	if mgr == nil or CAI_My2KDialog == nil then
		CAI_My2KDialog = nil
		return
	end

	for i, widget in ipairs(mgr.Stack) do
		if widget == CAI_My2KDialog then
			table.remove(mgr.Stack, i)
			break
		end
	end

	CAI_My2KDialog:Destroy()
	CAI_My2KDialog = nil

	if suppressRefocus then return end
...
local function CAI_PushDialog(dialog)
	CAI_RemoveCurrentDialog(true)
	CAI_My2KDialog = dialog
	if dialog ~= nil and mgr ~= nil then
		mgr:Push(dialog)
	end
end
...
local function CAI_CreateReadOnlyText(labelFunc, valueFunc)
	...
	if H ~= nil and H.SetEditBoxText ~= nil then
		H:SetEditBoxText(widget, valueFunc and valueFunc() or "")
	end
	return widget
end
...
local function CAI_CreateButton(labelControl, backingButton, fallback, onClick)
	...
	return mgr:CreateUIWidget("Button", {
		...
		IsDisabled = function()
			return backingButton ~= nil and backingButton.IsDisabled ~= nil and backingButton:IsDisabled()
		end,
		OnClick = function()
			if backingButton ~= nil and backingButton.IsDisabled ~= nil and backingButton:IsDisabled() then return end
	...
	local widget = mgr:CreateUIWidget("Edit", {
		...
		GetValue = function()
			if editControl ~= nil and editControl.GetText ~= nil then
				return editControl:GetText() or ""
			end
			return ""
		end,
		IsDisabled = function()
			return editControl ~= nil and editControl.IsDisabled ~= nil and editControl:IsDisabled()
		end,
		OnSetText = function(w, text)
			if editControl ~= nil and editControl.SetText ~= nil then
				editControl:SetText(text)
			end
			if options.OnChanged ~= nil then options.OnChanged(w, text) end
		end,
		OnCommit = function(w, text)
			...
			if options.OnChanged ~= nil then options.OnChanged(w, text) end
			if options.OnCommit ~= nil then options.OnCommit(w, text) end
		end,
		HighlightOnEdit = true,
	})
	if H ~= nil and H.SetEditBoxText ~= nil then
		H:SetEditBoxText(widget, editControl ~= nil and editControl.GetText ~= nil and editControl:GetText() or "")
	end
	return widget
end

local function CAI_AddIfPresent(list, widget)
	if widget ~= nil then table.insert(list, widget) end
end

local function CAI_MakeDialog(titleFunc, actionButtons, dlgContent)
	if H == nil or H.MakeGeneralDialog == nil then return nil end
	return H:MakeGeneralDialog(titleFunc, actionButtons, dlgContent)
end

local function CAI_ShowDialog(titleFunc, actionButtons, dlgContent)
	local dialog = CAI_MakeDialog(titleFunc, actionButtons, dlgContent)
	CAI_PushDialog(dialog)
end

local function CAI_BeginLogin(instance)
	if (FiraxisLive.My2KBeginLogin()) then
		UI.PlaySound("Play_MP_Player_Connect")
		instance.LoginButton:SetDisabled(true)
		if (instance.NewUserButton ~= nil) then
...
local function CAI_BeginNewUser(instance)
	if (FiraxisLive.My2KCreateNewUser()) then
		UI.PlaySound("Confirm_Bed_Positive")
		instance.LoginButton:SetDisabled(true)
		instance.NewUserButton:SetDisabled(true)
...
local function CAI_ShowMainMenu(instance)
	local content = {}
	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.LoginText, instance.LoginButton, "TXT_KEY_MY2K_ADDITION_LOGIN_USER", function()
		CAI_BeginLogin(instance)
	end))
	if instance.NewUserButton ~= nil then
		CAI_AddIfPresent(buttons, CAI_CreateButton(instance.NewUserText, instance.NewUserButton, "TXT_KEY_MY2K_ADDITION_SIGN_UP", function()
			CAI_BeginNewUser(instance)
		end))
	end
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.CancelText, instance.CancelButton, "TXT_KEY_MY2K_ADDITION_CANCEL", m_cancelFunction))
	CAI_ShowDialog(function() return CAI_GetText(instance.Title, "TXT_KEY_MY2K") end, buttons, content)
end

local function CAI_ShowUnlinkConfirmation(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.CancelText, instance.CancelButton, "TXT_KEY_MY2K_ADDITION_CANCEL", m_cancelFunction))
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.OkText, instance.OkButton, "TXT_KEY_MY2K_ADDITION_OK", function()
		if (FiraxisLive.My2KUnlinkAccount()) then
			UI.PlaySound("Play_MP_Player_Disconnect")
			instance.OkButton:SetDisabled(true)
			instance.CancelButton:SetDisabled(true)
	...
	CAI_ShowDialog(function() return CAI_GetText(instance.Title, "TXT_KEY_MY2K_ADDITION_UNLINK_ACCOUNT_TITLE") end, buttons, content)
end

local function CAI_ShowLogin(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	CAI_AddIfPresent(content, CAI_CreateEdit(function() return CAI_GetText(instance.EMailTitle) end, instance.Email, {
		OnChanged = function() ValidateLoginButtons(instance) end,
	}))
	CAI_AddIfPresent(content, CAI_CreateEdit(function() return CAI_GetText(instance.PasswordTitle) end, instance.Password, {
		OnChanged = function() ValidateLoginButtons(instance) end,
	}))

	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.CancelText, instance.CancelButton, nil, m_cancelFunction))
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.CreateText, instance.CreateButton, nil, function()
		local email = instance.Email:GetText()
		UI.PlaySound("Play_UI_Click")
		if email == nil or email == "" then email = " " end
		local password = instance.Password:GetText()
		if password == nil then password = "" end
		FiraxisLive.My2KLoginUser(email, password, true)
	end))
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.LoginText, instance.LoginButton, nil, function()
		if (instance.Email:GetText() ~= nil and IsValidEmail(instance.Email:GetText())) then
			local password = instance.Password:GetText()
			if password == nil then password = "" end
			FiraxisLive.My2KLoginUser(instance.Email:GetText(), password, false)
	...
	CAI_ShowDialog(function() return CAI_GetText(instance.Title) end, buttons, content)
end

local function CAI_ShowNewUser(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	CAI_AddIfPresent(content, CAI_CreateEdit(function() return CAI_GetText(instance.EMailTitle) end, instance.Email, {
		OnChanged = function() RealizeOkForNewUserDialog(instance) end,
	}))
	...
	CAI_ShowDialog(function() return CAI_GetText(instance.Title, "TXT_KEY_MY2K_ADDITION_SIGN_UP_TITLE") end, buttons, content)
end

local function CAI_ShowUserName(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	CAI_AddIfPresent(content, CAI_CreateEdit(function() return CAI_GetText(instance.EMailTitle) end, instance.Email, {
		OnChanged = function() RealizeOkForUserNameDialog(instance) end,
	}))

	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.CancelText, instance.CancelButton, nil, m_cancelFunction))
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.OkText, instance.OkButton, nil, function()
		if (instance.Email:GetText() ~= nil and IsValidEmail(instance.Email:GetText())) then
			UI.PlaySound("Play_UI_Click")
			m_bESCEnabled = false
			instance.OkButton:SetDisabled(true)
...
local function CAI_ShowLegal(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	...
	if m_currentDialogData ~= nil and m_currentDialogData.documents ~= nil then
		...
		for i, document in ipairs(m_currentDialogData.documents) do
			...
			docList:AddChild(mgr:CreateUIWidget("Button", {
				...
				OnClick = function()
					UI.PlaySound("Play_UI_Click")
					ShowLegalItemDialog(m_currentDialogData, documentIndex)
				end,
			}))
		end
		CAI_AddIfPresent(content, docList)
	end

	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.DisagreeAllText, instance.DisagreeAllButton, nil, function()
		UI.PlaySound("Play_UI_Click")
		local agreeList = {}
		agreeList["AgreeCount"] = 0
		if (FiraxisLive.My2KLegalResponse(agreeList)) then
	...
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.AgreeAllText, instance.AgreeAllButton, nil, function()
		UI.PlaySound("Play_UI_Click")
		local agreeList = {}
		agreeList["AgreeCount"] = #m_currentDialogData.documents
		for i, document in ipairs(m_currentDialogData.documents) do
...
local function CAI_GetLegalDocumentText()
	if m_currentDialogData == nil or m_currentDialogData.documents == nil then return "" end
	local document = m_currentDialogData.documents[m_legalItemIndex]
...
local function CAI_ShowLegalItem(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateReadOnlyText(function() return CAI_GetText(instance.Title) end, CAI_GetLegalDocumentText))
	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.OkText, instance.OkButton, "TXT_KEY_MY2K_ADDITION_OK", m_cancelFunction))
	CAI_ShowDialog(function() return CAI_GetText(instance.Title) end, buttons, content)
end

local function CAI_ShowMessage(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.OkText, instance.OkButton, nil, m_cancelFunction))
	CAI_ShowDialog(function() return CAI_GetText(instance.Title) end, buttons, content)
end

local function CAI_ShowLogout(instance)
	local content = {}
	CAI_AddIfPresent(content, CAI_CreateStaticText(function() return CAI_GetText(instance.Message) end))
	local buttons = {}
	CAI_AddIfPresent(buttons, CAI_CreateButton(instance.CancelText, instance.CancelButton, nil, m_cancelFunction))
...
local function CAI_RebuildCurrentDialog()
	if mgr == nil or H == nil then return end
	if m_currentDialog == nil then
		CAI_RemoveCurrentDialog()
		return
	end

	if m_currentDialogID == DIALOG_MAIN_MENU then
		CAI_ShowMainMenu(m_currentDialog)
	elseif m_currentDialogID == DIALOG_UNLINK_CONFIRMATION then
		CAI_ShowUnlinkConfirmation(m_currentDialog)
	elseif m_currentDialogID == DIALOG_LOGIN then
		CAI_ShowLogin(m_currentDialog)
	elseif m_currentDialogID == DIALOG_NEW_USER then
		CAI_ShowNewUser(m_currentDialog)
	elseif m_currentDialogID == DIALOG_USER_NAME then
		CAI_ShowUserName(m_currentDialog)
	elseif m_currentDialogID == DIALOG_LEGAL then
		CAI_ShowLegal(m_currentDialog)
	elseif m_currentDialogID == DIALOG_LEGAL_ITEM then
		CAI_ShowLegalItem(m_currentDialog)
	elseif m_currentDialogID == DIALOG_MESSAGE then
		CAI_ShowMessage(m_currentDialog)
	elseif m_currentDialogID == DIALOG_LOGOUT then
		CAI_ShowLogout(m_currentDialog)
	else
		CAI_ShowMessage(m_currentDialog)
	end
end
...
ContextPtr:SetInputHandler(function(input)
	if mgr ~= nil and mgr:HandleInput(input) then return true end
	if input:GetMessageType() == KeyEvents.KeyUp
		and input:GetKey() == Keys.VK_ESCAPE
		and m_bESCEnabled == true then
		if m_cancelFunction ~= nil then
			m_cancelFunction()
		else
			OnReturn()
...
ShowHideHandler = WrapFunc(ShowHideHandler, function(orig, bIsHide, bIsInit)
	local result = orig(bIsHide, bIsInit)
	if bIsHide then
		CAI_RemoveCurrentDialog()
	end
	return result
...
CreateLogoutDialog = WrapFunc(CreateLogoutDialog, function(orig)
	orig()
	CAI_RebuildCurrentDialog()
end)
