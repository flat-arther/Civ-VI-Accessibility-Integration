...
local m_CAISaveGameId:number = Input.GetActionId("SaveGame_CAI");

local function OnCAIInputActionStarted(actionId:number)
	if actionId == m_CAISaveGameId then
		if not CanLocalPlayerSaveGame() then
			Speak(Locale.Lookup("LOC_CAI_QUICK_SAVE_FAILED"));
			return true;
		end

		OnInputActionTriggered(m_QuicksaveId);
		Speak(Locale.Lookup("LOC_CAI_QUICK_SAVE_COMPLETE"));
		return true;
	end
...
Initialize = WrapFunc(Initialize, function(original)
	original();
